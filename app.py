...
def c1_prep_volcano_df(df, go_map, left_name, right_name, padj_thr, lfc_thr, strong_lfc=2.0):
    ...
    v["neglog10"] = -np.log10(v["padj_safe"])
...
def c1_make_percent_table(vdf: pd.DataFrame) -> pd.DataFrame:
    ...
    d["Dir"] = np.where(d["log2FoldChange"] > 0, f"{labs['left']} up", f"{labs['right']} up")
    d["GO_Domain"] = pd.Categorical(d["GO_Domain"].fillna("Unknown"),
                                    ["MF", "CC", "BP", "Unknown"], ordered=True)
    g = d.groupby(["Dir", "GO_Domain"], dropna=False).size().reset_index(name="N")
...
def c1_fig_volcano(vdf, padj_thr, lfc_thr, overlay_chemo, highlight_query):
    fig = px.scatter(
        vdf, x="log2FoldChange", y="neglog10", color="ColorKey",
    ...
    fig.add_vline(x=lfc_thr, line=dict(dash="dash", width=0.5))
    fig.update_layout(legend_title_text="", xaxis_title="Log2(FoldChange)",
                      yaxis_title="-log10(<i>padj</i>)", uirevision="volcano")
    if overlay_chemo:
        chemo = vdf[(vdf["is_sig"]) & (vdf["is_chemo"])].copy()
        if not chemo.empty:
            fig.add_trace(go.Scattergl(
                x=chemo["log2FoldChange"], y=chemo["neglog10"],
                mode="markers", name="Labeled (chemo)",
                marker=dict(color="black", size=7, opacity=0.9),
                hovertext=chemo["JoinKey"], hoverinfo="text"
            ))
    hits = c1_find_hits(vdf, highlight_query)
    if not hits.empty:
        fig.add_trace(go.Scattergl(
            x=hits["log2FoldChange"], y=hits["neglog10"],
...
def c1_render_go_tab(volcano_dfs, norm_means):
    ...
    for col, vdf, title_stub, fname in [
        ...
        with col:
            ...
            figp = px.bar(tbl, x="Dir", y="Percent", color="GO_Domain",
                          color_discrete_map=GO_COLS,
                          category_orders={"GO_Domain": ["MF", "CC", "BP", "Unknown"]},
                          text=tbl["Percent"].round(1).astype(str),
...
def h1_make_chemo_heatmap_matplotlib(
    ...
    # Optional search (matches OR2, GR1, etc - searches without the Hill prefix too)
    q = (search_term or "").strip().lower()
    if q:
        name_series = sub["Name_prefixed"].astype(str)
        mask = name_series.str.lower().str.contains(q) | name_series.str.lower().str.replace("hill", "", regex=False).str.contains(q)
        sub = sub[mask]
        if sub.empty:
            return None

    # Build selected columns in the same order: tissue blocks Ant, P, Leg and states MF, VF, Vm
    tissue_to_prefix = {"Antenna": "Ant", "M.palp": "P", "Tarsi": "Leg"}
    state_order = ["MF", "VF", "Vm"]

    cols = []
    col_labels = []
    for tissue_label in ["Antenna", "M.palp", "Tarsi"]:
    ...
    # Center on threshold:
    # below threshold -> negative (blue), threshold -> 0 (white), above -> positive (red)
    thr_log = float(np.log2(expr_thr + 1.0))
    mat_centered = mat_log - thr_log

    # Sort rows by max signal (so top genes are strongest)
    ...
    # Fixed-size figure (you can tweak these constants)
    n_rows = mat_centered.shape[0]
    n_cols = mat_centered.shape[1]
    fig_w = 6.5 + (n_cols * 0.45)
    ...
    ax.set_title(f"{fam} expression (log2(x+1) centered at threshold {expr_thr})", fontsize=12, pad=10)

    # Colorbar
    cbar = fig.colorbar(im, ax=ax, fraction=0.02, pad=0.02)
    cbar.set_label("log2(expr+1) - log2(threshold+1)", rotation=90)
...
def render_h1_tab():
    ...
    tissue_choices = st.multiselect(
        "Tissues (columns)",
        ["Antenna", "M.palp", "Tarsi"],
        default=["Antenna", "M.palp", "Tarsi"],
        key="h1_tissues_keep",
    )
    ...
    expr_thr = st.number_input(
        "Expression threshold (used for blue-white-red centering)",
        min_value=0.0, max_value=1e6, value=float(H1_THRESHOLD), step=1.0,
        key="h1_expr_thr",
    )
